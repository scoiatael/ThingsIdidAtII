...
typedef unsigned char setOfChars;

typedef struct __chomskygrammar__ {
  setOfChars fstType[26];
  unsigned int rule_nr[8];
  struct {
    unsigned char fst;
    unsigned char snd;
  } table[8][64];
...
{
  int i;
  forEachChar(i, printf("%d ", (*arg >> toSetIter(i)) & 1););
  printf("|");
}
...
void computeZipSetsWithGrammar(chGrammar* grammar, setOfChars setA, setOfChars setB, setOfChars* setC)
{
  int i,j;
  forEachChar(i, 
...
void initGrammar(chGrammar* arg)
{
  ...
  for (i = 0; i < 256; i+=1) {
    for (j = 0; j < 256; j+=1) {
      computeZipSetsWithGrammar(arg, (unsigned char)i, (unsigned char)j, &arg->quickZip[i][j]); 
    }
  }
}
...
int load_chGrammar(chGrammar* arg, FILE* file)
{
  ...
  /*
  for (i = 0; i < 256; i++) {
    memset(arg->quickZip[i], False, 256);
  }
  */
  int fstnr, sndnr;
...
void zipSetsWithGrammar(chGrammar* grammar, setOfChars* setA, setOfChars* setB, setOfChars* setC)
{
  ...
  *setC = grammar->quickZip[*setA][*setB];
}
...
char word_chGrammar(chGrammar* grammar, char word[512], int wordlength)
{
 ...
 //     printf("%d %d: ", i, j);
      //print_setOfChars(&__AuxTable__[i][j]);
    }
//    printf("\n");
  }
#endif
